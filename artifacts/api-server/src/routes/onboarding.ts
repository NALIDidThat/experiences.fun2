...
router.post("/onboarding/complete", async (req: Request, res: Response): Promise<void> => {
  ...
  const { name, username, city, country, interests, role, bio, user_type, node_name, node_type } = parsed.data;
      ...
      try {
        const [updated] = await db
          ...
          .set({
            ...
            user_type: user_type || existingByPrivy.user_type,
            node_name: node_name ?? existingByPrivy.node_name,
            node_type: node_type ?? existingByPrivy.node_type,
          })
      ...
      try {
        const [updated] = await db
          ...
          .set({
            ...
            user_type: user_type || existingByTelegram.user_type,
            node_name: node_name ?? existingByTelegram.node_name,
            node_type: node_type ?? existingByTelegram.node_type,
          })
    ...
    try {
      const [updated] = await db
        ...
        .set({
          ...
          user_type: user_type || req.currentUser.user_type,
          node_name: node_name ?? req.currentUser.node_name,
          node_type: node_type ?? req.currentUser.node_type,
        })
  ...
  try {
    ...
    if (verifiedTelegramId) {
      ...
      if (botToken) {
        const roleLabel = user_type === "educator" ? "Your node is live" : "Start exploring experiences";
        await sendDM(
          botToken,
          verifiedTelegramId,
          `Welcome to experiences.fun, ${name}! 🎉\n\nYou've earned +50 XP for completing your profile. ${roleLabel} near ${city}!`,
          miniAppUrl ? { label: "Open App", url: miniAppUrl } : undefined,
        );
      }
    }
